...
namespace SickLeaveDetails.Profiles
{
    public class SickLeaveMapper : Profile
    {
        private readonly IHttpContextAccessor _httpAccessor;

        public SickLeaveMapper(IHttpContextAccessor httpAccessor)
        {
            _httpAccessor = httpAccessor;

            // SickLeave -> GetAllSickLeaveQueryResponse map'lemesi
            CreateMap<SickLeave, GetAllSickLeaveQueryResponse>()
                .ForMember(dest => dest.Employee, opt => opt.MapFrom(src => src.Employee))
                .ReverseMap();

            // SickLeave -> GetByIdSickLeaveQueryResponse map'lemesi
            CreateMap<SickLeave, GetByIdSickLeaveQueryResponse>().ForMember(dest => dest.Employee, opt => opt.MapFrom(src => src.Employee))
                .ReverseMap();
